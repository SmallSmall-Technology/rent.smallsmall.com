...
class ResidentialInspectionController extends Controller
{
    public function store(Request $request)
    {
        ...
        $residentialInspection = new ResidentialInspection([
            'inspection_time' => date('Y-m-d H:i:s', $inspection_time),
            'residential_id'  => $params->residential_id
        ]);

        $user->residential_inspection()->save($residentialInspection);
        $user->refresh();

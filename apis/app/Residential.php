...
class Residential extends Model implements Payable
{
    /**
    ...
    public function pre_payment_checks($params)
    {
        ...
        if(($this->isRented()) && (!$this->belongsToUser($params->user_id))){
                $this->pre_payment_errors = ['error' => 
                'This Residential is not available for rent.'];
        }
    ...
    public function favorites()
    {
     ...
     static function get_images($residential){
        ...
        $image_path = $base_url.$residential->folderName.'/'.'properties';
        $images = [];
